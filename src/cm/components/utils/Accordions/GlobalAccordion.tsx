...
const GlobalAccordion = (props: AccordiongPropType & {id: string}) => {
  ...
  const [open, setopen] = useJotaiByKey<atomTypes[`globalAccordionOpen`]>(`globalAccordionOpen`, null)
      ...
      >
        <div className={` w-full justify-between `}>
          <R_Stack className={`flex-nowrap`}>
            <ChevronRightIcon className={cl(` h-5 font-bold duration-300`, open ? ` rotate-90  ` : ` rotate-0`)} />
            {label}
          </R_Stack>
        </div>
      </div>
