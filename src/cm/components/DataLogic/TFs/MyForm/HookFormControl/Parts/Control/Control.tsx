...
import ErrorMessage from 'src/cm/components/DataLogic/TFs/MyForm/HookFormControl/Parts/Control/ErrorMessage'
import {cl, funcOrVar} from 'src/cm/lib/methods/common'
import Label from 'src/cm/components/DataLogic/TFs/MyForm/HookFormControl/Parts/Control/Label'
import VariousInput from 'src/cm/components/DataLogic/TFs/MyForm/HookFormControl/Parts/Control/VariousInput'

import {ControlContextType, ControlPropType} from '@cm/types/form-control-type'
...
const Control = React.memo((props: ControlPropType) => {
  const {
    wrapperId,
    flexDirection,
    wrapperClass,
    ...
    return (
      <div className={shownButDisabled ? 'pointer-events-none' : ''}>
        <R_Stack className={`w-full flex-nowrap gap-0.5 `}>
          ...
          {/* main */}
          {col.form?.editFormat?.({...controlContextValue}) ?? <VariousInput {...{controlContextValue}} />}
