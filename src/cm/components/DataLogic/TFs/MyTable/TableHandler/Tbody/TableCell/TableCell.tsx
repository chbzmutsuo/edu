...
import {getValue} from 'src/cm/components/DataLogic/TFs/MyTable/TableHandler/Tbody/TableCell/lib/getValue'

import TdContent from '@components/DataLogic/TFs/MyTable/TableHandler/Tbody/TableCell/childrens/TdContent'
  ...
  (props: {
    ...
    return (
      <td key={`${col?.id}_dbId${record?.id}_index${columnIdx}`} {...tdProps}>
        <div className={`flex h-full w-full items-start `}>{TdContentMemo}</div>
      </td>
    )
  }
)
