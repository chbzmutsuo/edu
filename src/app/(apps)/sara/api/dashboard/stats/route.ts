...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // 今日の申請
      prisma.saraEvaluationRequest.findMany({
        where: {
          ...baseWhere,
      ...
      // 総申請数
      prisma.saraEvaluationRequest.count({
        where: baseWhere
      }),

      // 家族の子どもリスト（親の場合のみ）
      auth.type === 'parent' ? prisma.saraChild.findMany({
    ...
    const stats = {
      ...
      week: {
        ...
        totalScore: weekRequests
          .filter(r => r.status === 'approved')
          .reduce((sum, r) => sum + r.evaluationScore.score, 0)
      },
      month: {
        total: monthRequests.length,
        approved: monthRequests.filter(r => r.status === 'approved').length,
        totalScore: monthRequests
          .filter(r => r.status === 'approved')
          .reduce((sum, r) => sum + r.evaluationScore.score, 0)
      },
      pending: {
