    ...
    try {
      ...
      const result = await doStandardPrisma('healthRecord', 'findMany', {
        where: {
          userId: session.id,
          recordDate: {
            gte: getMidnight(today),
            lt: Days.day.add(getMidnight(today), 1),
          },
        },
        include: {
          Medicine: true,
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">📈 今日の記録数</h2>
                  ...
                  {recentRecords.map((record, index) => (
                    <tr key={record.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-3 text-sm text-gray-600">
                        ...
                        {(record.category === 'urine' || record.category === 'stool' || record.category === 'meal') && '記録済み'}
