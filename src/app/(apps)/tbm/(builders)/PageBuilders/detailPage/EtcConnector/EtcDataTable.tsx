      ...
      if (record.isGroupHeader) {
          ...
          csvTableRow: [
            {
              label: '',
              cellValue: (
                <Button size="sm" variant="ghost" onClick={() => handleUngroupRecords(record.meisaiId)} disabled={isLoading}>
                  解除
                </Button>
              ),
              cellProps: {
        ...
        return {
          csvTableRow: [
            {
              label: '',
              cellValue: <Checkbox checked={!!selectedRows[record.id]} onCheckedChange={() => toggleRowSelection(record.id)} />,
            },
            {
              label: '利用年月日（自）',
              cellValue: Days.format.formatDate(record.fromDate),
