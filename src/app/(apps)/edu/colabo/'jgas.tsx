    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">授業を作成</button>
        </div>
            ...
            {myGames.length > 0 ? (
              myGames.slice(0, 3).map(game => (
                <div key={game.id} className="border rounded p-2">
                  ...
                  <div className="text-sm text-gray-500">{new Date(game.date).toLocaleDateString('ja-JP')}</div>
                </div>
              ))
            ) : (
