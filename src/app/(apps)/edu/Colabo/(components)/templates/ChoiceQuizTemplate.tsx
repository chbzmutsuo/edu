  ...
  const getChoiceStats = () => {
    ...
    choiceBlocks.forEach(choice => {
      stats[choice.id] = {
        ...
        percentage:
          responses.length > 0
            ? Math.round((responses.filter(r => r.choiceAnswer === choice.id).length / responses.length) * 100)
            : 0,
    ...
    <div className="max-w-4xl mx-auto p-8">
                      ...
                      <div className="flex-1">
                        ...
                        {isTeacher && showResults && stats && (
                          <div className="mt-2 flex items-center space-x-4">
                            ...
                            {stats.isCorrect && <span className="text-green-600 text-sm font-medium">正解</span>}
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {!isTeacher && !isPreview && !hasSubmitted && (
        <div className="text-center">
          <Button onClick={handleSubmit} disabled={!selectedChoice} size="lg">
            回答を送信
          </Button>
        </div>
      )}
      ...
      {!isTeacher && hasSubmitted && (
        <div className="text-center p-6 bg-green-50 rounded-lg">
          ...
          <div className="text-green-600 text-sm mt-1">先生が結果を発表するまでお待ちください</div>
        </div>
      )}
      ...
      {isTeacher && !isPreview && (
        <div className="mt-8 p-4 bg-gray-100 rounded-lg">
          ...
          <div className="flex space-x-2">
            <Button onClick={() => onShowResults && onShowResults(!showResults)}>
