...
const formatEvent = (event) => {
  ...
  switch (event.name) {
    ...
    case 'webpack-invalidated-server':
      head = `${pc.bold(`${event.name.slice(-6)} recompilation`)} ${event.tags.trigger === 'manual'
          ? '(new page discovered)'
    ...
    default:
      if (event.name.startsWith('build-module-')) {
        ...
        head = `${pc.magenta('module')} ${packageName
            ? `${pc.bold(pc.cyan(packageName))} (${formatFilename(event.tags.name)}${mergedChildren ? ` + ${mergedChildren}` : ''
            })`
